...
!**********************************************************************
subroutine split_line(line,nargs,ind_ini,ind_end)
  implicit none
  character*100                 :: line
  integer                       :: i, n, j, k, nblank, nargs, nargs_found
  character*1                   :: current, previous
  ...
  do i=1,n
     current=line(i:i)
     if (current.ne.' ') then
        if (previous.eq.' ') nargs_found=nargs_found+1
     endif
     if (current.eq.' ') nblank=nblank+1
     previous=current
  enddo
  !
  if (nargs.ne.nargs_found) then
     print*,'Problems reading observed spectral line information. STOP'
     print*,nargs,nargs_found,line
     STOP
  endif
  !
  previous=' '

MODULE ATOM_DATABASE
  !
  ! J M Borrero (March 25, 2013)
  ! A Pastor: added from elements 72 to 92 (June 7, 2017)
  !
  ! Basically cut/paste from atmdatb.f in SIR
  ! which is basically the same as provided by Axel Wittmann
  ! Note: derivatives are of the natural logarith of the
  ! partition function
  !
  USE CONS_PARAM
  INTEGER,  PARAMETER                   :: NELEM = 92
  REAL(SP), DIMENSION(NELEM)            :: XI, XII, MATOM, ABUND
...
CONTAINS
  ...
  SUBROUTINE ATOM_INIT
    ...
    XI(:)=(/0.754,24.58,5.39,9.32,8.298,11.256,14.529,13.614,17.418, &
  ...
  PURE SUBROUTINE PARTITION_FUNCTION(INDEX,T,UI,UII,UIII,DUI,DUII,DUIII)
    ...
    REAL(SP),   INTENT(IN)                :: T
    REAL(SP),   INTENT(OUT)               :: UI, UII, UIII
    REAL(SP),   INTENT(OUT)               :: DUI, DUII, DUIII
    REAL(SP)                              :: X, Y, DX
    !
    UI=0.
    UII=0.
